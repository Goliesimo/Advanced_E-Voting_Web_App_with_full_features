...
// Define the directory to store profile pictures relative to the current script
$uploadDirectory = dirname(__FILE__) . '/profile_pictures/';
...
if ($_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($profilePicture['tmp_name'], $uploadDirectory . $filename)) {
        ...
        if ($stmt->execute([$username, $hashedPassword, $email, $filename])) {
            // Registration successful
            echo "<h3>Registration successful.<br> Now click login page</h3>";
            echo '<a href="login.php" class="btn btn-primary">Login</a> | <a href="../index.php">Go to Homepage</a>';
            exit();
        } else {
