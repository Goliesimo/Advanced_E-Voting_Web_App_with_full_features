  ...
  <body>
    <div class="container-scroller">
      ...
      <!-- partial -->
        ...
        <nav class="navbar p-0 fixed-top d-flex flex-row mt-0" style="background: transparent;">
...
if ($stmt->rowCount() > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<td>";
echo "<button type='button' class='btn btn-outline-primary' data-toggle='modal' data-target='#accreditModal{$row['voter_id']}' id='accreditBtn{$row['voter_id']}' onclick='handleAccreditationConfirmation(\"{$row['voter_id']}\")'>";
echo $row['status'] === 'unaccredited' ? 'Accredit' : 'Unaccredit';
echo "</button>";
echo "</td>";
...
xhr.onreadystatechange = function() {
    if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        alert('Accreditation for user with VoteID is Successfull ' + voterId);
    }
};

// Send an AJAX request to update the accreditation status
xhr.open('POST', 'approve_accreditation.php', true);
